...
public class NewGameOptions {
	...
	/**
	 * Constructor that takes a MainWindow object and sets this window to be the
	 * parentWindow of this new frame.
	 * 
	 * @param mainWindow
	 */
	...
	private void setComponentsText() {
		/*
		 * Setting up player 1's name,the contents of the drop down menus and
		 * the names of the check boxes
		 */
		player1Name.setText("Player 1");
		player1DifficultyComboBox
				.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Easy", "Medium", "Hard" }));

		/*
		 * Setting up the fields to be displayed, default player 1 human ticked
		 * and the AI fields are disabled
		 */
		player1DifficultyComboBox.setEnabled(false);

		/*
		 * Setting up player 2's name,the contents of the drop down menus and
		 * the names of the check boxes
		 */
		player2Name.setText("Player 2");
		player2DifficultyComboBox
				.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Easy", "Medium", "Hard" }));

		// Populates the player type combo boxes with the appropriate data
		player1TypeComboBox.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Human", "AI" }));
		player2TypeComboBox.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Human", "AI" }));
	...
	private void addComponentsToFrame(GridBagConstraints layout) {
		...
		/*
		 * Insets the player label away from the box boundary and away from the
		 * left boundary grid space
		 */
		...
		/*
		 * Sets the inset for each of the player type combo boxes, so they are
		 * away from the edge
		 */
		...
		/*
		 * Insets all the AI difficulty boxes from the outside boundary and
		 * brings them in from the right so they dont fill up their grid space
		 */
		...
		layout.insets = new Insets(0, 40, 0, 40); // shrinks the button on both
													// sides
	 ...
	 */
	private GridBagConstraints setXYAndWeight(GridBagConstraints layout, int x, int y, double weight, double weighty) {
		layout.gridx = x;
		layout.gridy = y;
		if (weight != -1){
	 ...
	 * @return layout same gridbagconstraints objects but weightx and weighty
	 *         have been set to 0
	 */
	...
	/**
	 * A private method for setting up the listeners for all the components on
	 * the screen
	 */
	private void setUpListeners() {
		...
		/*
		 * Sets up code for the the combo boxes, enabling/disabling the
		 * appropraite fields
		 * 
		 */
		...
		timerCheckBox.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				/*
				 * Checks whether or not the timer is selected, if it is then it
				 * disables it otherwise it enables it
				 * 
				 */
	...
	/**
	 * This method will create the ViewPlayer objects from the data supplied by
	 * the form objects and pass them into the main window instance
	 * (myParentWindow). Also checks that the username is not over the specified
	 * limit
	 * 
	 */
	private void createNewGame() {
		...
		if (player1Name.getText().length() >= TEXTFIELD_LIMIT) {
			...
			JOptionPane.showMessageDialog(new JFrame(),
					"Player 1 name is too long, can be a maximum of " + TEXTFIELD_LIMIT + " characters");
			return;
		} else if (player2Name.getText().length() >= TEXTFIELD_LIMIT) {
			player2Name.setForeground(Color.RED);
			JOptionPane.showMessageDialog(new JFrame(),
					"Player2 name is too long, can be a maximum of " + TEXTFIELD_LIMIT + "characters");
			return;
		}
		else{
			//Do nothing.
		...
		ViewPlayer player1 = new ViewPlayer(player1Name.getText(), getDepth(player1DifficultyComboBox),
				getIntFromDifficulty(player1DifficultyComboBox), !isPlayerAI(player1TypeComboBox));

		ViewPlayer player2 = new ViewPlayer(player2Name.getText(), getDepth(player2DifficultyComboBox),
				getIntFromDifficulty(player2DifficultyComboBox), !isPlayerAI(player2TypeComboBox));

		/*
		 * Checks if the timer is selected, if its not it makes timer = -1,
		 * otherwise it makes it equal to the time in the field in milliseconds
		 */
	...
	/**
	 * Given a combo box, if the selected item is easy it will return 1
	 * otherwise it will return 2 (medium)
	 * 
	...
	/**
	 * Given a combobox will take out the text and check if it contains the
	 * specified difficulties, easy, medium or hard and return a depth dependant
	 * on the text in the combo box
	 *
	...
	private int getDepth(JComboBox difficultyComboBox) {
		...
		String textDifficulty = (String) difficultyComboBox.getSelectedItem();
		if (textDifficulty.equalsIgnoreCase("hard")) {
			return HARD_DIFFICULTY_DEPTH;
		}
		else if (textDifficulty.equalsIgnoreCase("medium")) {
			return MEDIUM_DIFFICULTY_DEPTH;
		}
		else {
	...
	/**
	 * Returns true if the player type, passed in via the selected combo box is
	 * "AI" False otherwise
	 * 
	 * @param thisComboBox
	 * @return
