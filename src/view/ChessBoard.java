...
class ChessBoard extends JPanel {
	...
	public void paintComponent( Graphics g ) {
		...
		for ( int i = 0; i < cols; i++ ) {
			...
			int vertSpaceLeft = (int) ( SCALING_TO_HALF * rectSize + SCALING_TO_HALF * fontSize );
			int vertSpaceBottom = (int) ( ( 1 + SCALING_TO_HALF ) * fontSize );
			int horSpaceBottom = (int) ( SCALING_TO_HALF * rectSize - SCALING_TO_HALF * fontWidth );
			// paint the legend to the left and the bottom
			Font f = new Font("Dialog", Font.PLAIN, fontSize);
			g.setColor( Color.BLACK );
			g.setFont( f );
			g.drawString( legendAlph[a], cellcol + horSpaceBottom, rectSize * CELL_AMOUNT + vertSpaceBottom );
			g.drawString( legendNum[CELL_AMOUNT - 1 - a], 5, (i) * rectSize + vertSpaceLeft );
	...
	private void setHighlightPosition() {
		...
		if ( activePosition != null ) { 
			...
			boardRep.fillRect( cellcol + 1 + LEFTLEGEND_SPACE, cellrow + 1, rectSize, rectSize );
		}
	...
	private void setHighlightLast() {
		...
		if ( lastMove != null ) {
			...
			int x1 = ( x ) * rectSize + ( rectSize / DOUBLE_COLUMN_DIVISOR ) + LEFTLEGEND_SPACE;
			int y1 = ( y ) * rectSize + ( rectSize / DOUBLE_COLUMN_DIVISOR );
			int x2 = ( xx ) * rectSize + ( rectSize / DOUBLE_COLUMN_DIVISOR ) + LEFTLEGEND_SPACE;
			int y2 = ( yy ) * rectSize + ( rectSize / DOUBLE_COLUMN_DIVISOR );

			Graphics2D g2 = ( Graphics2D ) boardRep;
			g2.setColor( previousBG );
			g2.setStroke( new BasicStroke( PREVISOU_LINE_WIDTH, BasicStroke.CAP_ROUND, BasicStroke.JOIN_MITER ) );
